...
- CSRF / Origin validation on AI route — rejects cross-origin browser requests (403)
- Per-IP rate limiting (30 req/min) with `Retry-After` and `X-RateLimit-*` response headers
- Automatic rate-limit map cleanup every 5 minutes to prevent unbounded memory growth
- All API request bodies validated via `AiRequestSchema.safeParse()` — Zod 4 replaces manual type-casting
- Input sanitisation on AI text and history
