...
/**
 * If Firebase credentials are not configured (e.g. fresh clone, CI build,
 * or local demo without secrets), fall back to harmless placeholder values
 * so the app can still build and render. Auth/Firestore calls will fail
 * gracefully at runtime instead of crashing the entire build.
 */
...
if (!hasFirebaseConfig) {
  logger.warn(
    { module: "Firebase" },
    "Firebase environment variables are not set. Using placeholder config. " +
    "Auth and Firestore features will be disabled until NEXT_PUBLIC_FIREBASE_* " +
    "variables are configured. See .env.example for reference."
  );
