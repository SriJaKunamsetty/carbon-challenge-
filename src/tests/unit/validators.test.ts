...
describe("AiRequestSchema", () => {
  it("accepts a minimal valid parser request", () => {
    const result = AiRequestSchema.safeParse({ text: "I drove 10km" });
    expect(result.success).toBe(true);
    if (result.success) {
      expect(result.data.mode).toBe("parser"); // default
      expect(result.data.history).toEqual([]); // default
    }
  });

  it("accepts a chat mode request with history and profile", () => {
    const result = AiRequestSchema.safeParse({
      text: "How do I reduce my footprint?",
      mode: "chat",
      history: [{ role: "user", content: "Hello" }],
      profile: { name: "Sri", carbonScore: 75 },
    });
    expect(result.success).toBe(true);
  });

  it("rejects empty text", () => {
    const result = AiRequestSchema.safeParse({ text: "" });
    expect(result.success).toBe(false);
    if (!result.success) {
      expect(result.error.errors[0]?.message).toBe("Text is required");
    }
  });

  it("rejects text exceeding 5000 characters", () => {
    const result = AiRequestSchema.safeParse({ text: "x".repeat(5001) });
    expect(result.success).toBe(false);
    if (!result.success) {
      expect(result.error.errors[0]?.message).toContain("exceeds maximum length");
    }
  });

  it("rejects invalid mode values", () => {
    const result = AiRequestSchema.safeParse({ text: "hello", mode: "invalid_mode" });
    expect(result.success).toBe(false);
  });

  it("rejects mode: null", () => {
    const result = AiRequestSchema.safeParse({ text: "hello", mode: null });
    expect(result.success).toBe(false);
  });

  it("trims whitespace from text before min length check", () => {
    // "   " trims to "" which has length 0 → fails min(1)
    const result = AiRequestSchema.safeParse({ text: "   " });
    expect(result.success).toBe(false);
  });

  it("rejects missing text field", () => {
    const result = AiRequestSchema.safeParse({ mode: "parser" });
    expect(result.success).toBe(false);
  });
});
