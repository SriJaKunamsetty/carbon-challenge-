...
export function AuthProvider({ children }: AuthProviderProps) {
  ...
  const contextValue = useMemo(
    ...
    // The auth action callbacks (loginWithGoogle, loginWithEmail, etc.) are defined
    // inside this component but never change identity — they close over Firebase SDK
    // references that are module-level singletons. Including only the reactive state
    // values (user, profile, loading) avoids unnecessary context re-renders while
    // keeping all consumers up to date with auth state changes.
